...
VERSIÓN    : 20.0 (ARQUITECTURA CLOUD INTEGRADA SOBRE CÓDIGO MAESTRO)
=============================================================================
"""
...
# Importación limpia: el cazador ya no llama al megáfono para TNS
...
# =====================================================================
# FUNCIONES PUENTE PARA LA NUBE (GCS)
# =====================================================================
...
# =====================================================================
# FUNCIONES DEL PUENTE JSON (MODIFICADAS PARA CLOUD)
# =====================================================================
def cargar_catalogo_maestro():
    """Carga el catálogo histórico en memoria viva para actualizarlo sin perder datos anteriores."""
    if not MODO_LOCAL and bucket:
        blob = bucket.blob(CATALOGO_JSON)
        if blob.exists():
            try:
                lista = json.loads(blob.download_as_text())
...
def guardar_catalogo_maestro(catalogo_dict):
    """Vuelca la memoria viva al disco para alimentar a Streamlit y al Bucket GCS si aplica."""
    # 1. Guardado Local (Como respaldo)
    ...
    if not MODO_LOCAL and bucket:
        try:
            blob = bucket.blob(CATALOGO_JSON)
            blob.upload_from_filename(CATALOGO_JSON, content_type='application/json')
        except Exception: pass
...
# =====================================================================
# RUTINA DE CAZA: TNS (SURVEY GLOBAL)
# =====================================================================
def consultar_tns_sur(mjd_reciente, client, catalogo_dict):
    log_file = "bitacoras/bitacora_TNS_GLOBAL.log"
    
    TNS_BOT_ID = os.getenv("TNS_BOT_ID")
    TNS_API_KEY = os.getenv("TNS_API_KEY")
    
    if not TNS_BOT_ID or not TNS_API_KEY:
        registrar_log("[!] ERROR FATAL: Credenciales TNS no encontradas en el archivo .env", log_file)
        return
        
    headers = {'User-Agent': f'tns_marker{{"tns_id":{TNS_BOT_ID}, "type":"bot", "name":"Magallanes_Bot"}}'}

    supernovas_procesadas = set()
    if os.path.exists(MEMORIA_TNS):
        with open(MEMORIA_TNS, 'r', encoding='utf-8') as f:
            supernovas_procesadas = set(linea.strip() for linea in f.readlines() if linea.strip())

    payload = {"api_key": TNS_API_KEY, "data": json.dumps({"classified_sne": 1, "order": "desc", "order_by": "discoverydate"})}
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if isinstance(datos, dict) and 'data' in datos:
                if isinstance(datos['data'], dict) and 'reply' in datos['data']:
                    lista_eventos = datos['data']['reply']
                elif isinstance(datos['data'], list):
                    lista_eventos = datos['data']
            elif isinstance(datos, list):
                lista_eventos = datos
                
            if not lista_eventos:
                registrar_log("[-] TNS respondió OK (200), pero la lista de eventos está vacía o ilegible.", log_file)
                return
                
            registrar_log(f"[+] Base de datos TNS capturada. Eventos totales recibidos: {len(lista_eventos)}", log_file)
            
            lista_eventos = lista_eventos[-300:]
            lista_eventos.reverse()
            
            for evento in lista_eventos:
                if isinstance(evento, dict):
                    objname, prefix = str(evento.get('objname', '')), str(evento.get('prefix', 'SN'))
                    if not objname: continue
                    id_evento = f"{prefix} {objname}".strip()
                    
                    if id_evento in supernovas_procesadas: continue
                        
                    registrar_log(f"Descargando ficha para {id_evento}...", log_file)
                    print(f"    [+] Novedad TNS detectada: {id_evento}")
                    try:
                        ...
                        if resp_obj.status_code == 200:
                            datos_obj = resp_obj.json().get('data', {}).get('object') or resp_obj.json().get('data', {})
                            ra_str, dec_str = str(datos_obj.get('ra', 'Desconocida')), str(datos_obj.get('dec', 'Desconocida'))
                            ra_float = float(datos_obj.get('radeg') or evento.get('radeg') or 0.0)
                            dec_float = float(datos_obj.get('decdeg') or evento.get('decdeg') or 0.0)
                    ...
                    # =======================================================
                    # BLOQUE MODIFICADO: REGISTRO TNS OFICIAL (SIN ATEL)
                    # =======================================================
                    id_puro = id_evento.replace("SN ", "").replace("AT ", "").strip()
                    url_tns = f"https://www.wis-tns.org/object/{id_puro}"
...
======================================================================"""
                    ...
                    # INYECCIÓN DIRECTA AL CATÁLOGO JSON
                    try:
            ...
            registrar_log(f"[!] EL SERVIDOR TNS RECHAZÓ LA CONEXIÓN. Código HTTP: {response.status_code}", log_file)
            print(f"[!] EL SERVIDOR TNS RECHAZÓ LA CONEXIÓN. Código HTTP: {response.status_code}")
            
    except Exception as e: 
        registrar_log(f"[!] Error Crítico de red al conectar con TNS: {e}", log_file)
        print(f"[!] Error Crítico de red al conectar con TNS: {e}")

# =====================================================================
# BUCLE PRINCIPAL (MAIN)
# =====================================================================
def main():
    print("=== INICIANDO CAZADOR MULTIPROPÓSITO (VERSIÓN ORIGINAL + TAP ADQL) ===")
    client = Alerce()
    mjd_reciente = obtener_mjd_rastreo()
    url_tap = "https://tap.alerce.online/tap"
    servicio_tap = vo.dal.TAPService(url_tap)
    
    # Cargar la memoria viva del catálogo antes de iniciar el ciclo
    ...
    for current_survey in ["ZTF", "LSST", "TNS_GLOBAL"]:
        ...
        if current_survey == "TNS_GLOBAL":
            consultar_tns_sur(mjd_reciente, client, catalogo_dict)
            continue
            
        target_classes = diccionario_categorias.get(current_survey, [])
        limite_mjd = mjd_reciente - 0.2
        
        # --- BÚSQUEDA MASIVA VÍA ADQL (BIFURCADA POR ENCUESTA) ---
        ...
        try:
            ...
            print(f"   ({len(candidatos)} objetos capturados. Iniciando análisis de telemetría individual...)")
            ...
            for index, fila in candidatos.iterrows():
                ...
                try:
                    ...
                    if clase_ia_final in target_classes and probabilidad > 0.60:
                        ...
                        # =======================================================
                        # BLOQUE MODIFICADO: FORMATO LISTO PARA INYECCIÓN EN LAB
                        # =======================================================
                        texto_reporte = f"""
...
"""
                        ...
                        # Ya NO lo guardamos como REPORTE_ALERTA_{oid}.txt
                        # Lo pasamos inyectado directo al pipeline
    ...
    # Volcar la memoria viva actualizada al archivo JSON para Streamlit
